...
public class MultiBoxTracker {
    ...
    public synchronized void draw(final Canvas canvas, final Integer facing) {
        ...
        faces_numb = trackedObjects.size();
        float[] X_f = new float[5];
        float[] Y_f = new float[5];
        RectF[] face_rectF = new RectF[5];
        int sum_x = 0, sum_y = 0;
        String face_info = "";
        String mode = "";
        logger.i("얼굴 갯수 : " + faces_numb);
        ...
        if(faces_numb >= 6){
            ...
            for (int i = 0; i < faces_numb; ++i) {
                ...
                face_rectF[i] = trackedPos;
                // x 좌표와 y 좌표가 바뀜. 정확한 이유는 아직 모르겠음.
                Y_f[i] = face_rectF[i].centerX();
                X_f[i] = face_rectF[i].centerY();
                sum_x += X_f[i];
                sum_y += Y_f[i];

                fh[i] = (int) trackedPos.height();

                logger.i("Face" + i + " Position : " + X_f[i] + "/" + Y_f[i]);
            }
            ...
            logger.i("얼굴 높이 : " + fh[max_idx_fh]);

            if(fh[max_idx_fh] > 100){
                distanceMode = "N";
            }else if(fh[max_idx_fh] > 70){
                distanceMode = "M1";
            }else if(fh[max_idx_fh] > 40){
                distanceMode = "M2";
            }else{
                distanceMode = "M3";
            }
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            //TODO: Delete these loggers
            logger.i("connersize : " + cornerSize);
            logger.i("trackedpos left : " + trackedPos.left);
            logger.i("trackedpos inverse left : " + (1400 - trackedPos.left));
            ...
            if(facing == 1) {
                ...
                canvas.drawRoundRect(1400-trackedPos.right, trackedPos.top, 1400-trackedPos.left, trackedPos.bottom, cornerSize, cornerSize, boxPaint);
                borderedText.drawText(canvas, 1400 - (trackedPos.left + cornerSize), trackedPos.bottom, labelString);
            }
        }
    }
